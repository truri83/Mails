...
Public Function GetOderErstelleKontakt(ByVal strName As String, _
    ...
    Set rs = db.OpenRecordset( _
        "SELECT KontaktID FROM " & TBL_KONTAKTE & " WHERE Anzeigename='" & SQLSafe(strName) & "'", dbOpenSnapshot)
    If Not rs.EOF Then
        lngID = rs!KontaktID
        rs.Close: Set rs = Nothing: Set db = Nothing
        GetOderErstelleKontakt = lngID
    ...
    With rs
        ...
        !KontaktTyp = IIf(IstSystemEmail(strEmail), "System", "Person")
        !ErstelltAm = Now
        !AktualisiertAm = Now
        .Update
        .Bookmark = .LastModified
...
Public Function SpeichereOrdner(ByVal strName As String, _
    ...
    varID = DLookup("OrdnerID", TBL_ORDNER, "OrdnerPfad='" & SQLSafe(strPfad) & "'")
    If Not IsNull(varID) Then
        ' Aktualisieren: Elementanzahl + LetzterSync
        db.Execute "UPDATE " & TBL_ORDNER & " SET " & _
    ...
    ' Neu anlegen
    Set rs = db.OpenRecordset(TBL_ORDNER, dbOpenDynaset)
    With rs
        .AddNew
...
Public Function GetOderErstelleThread(ByVal strBetreff As String, _
    ...
    If strInReplyTo <> "" Then
        Set rs = db.OpenRecordset( _
            "SELECT ThreadID FROM " & TBL_THREADS & " WHERE ThreadIdentifier='" & SQLSafe(strInReplyTo) & "'", _
            dbOpenSnapshot)
        If Not rs.EOF Then
            lngID = rs!ThreadID
            rs.Close: Set rs = Nothing
    ...
    Set rs = db.OpenRecordset( _
        "SELECT ThreadID FROM " & TBL_THREADS & " WHERE ThreadIdentifier='" & SQLSafe(strIdent) & "'", _
        dbOpenSnapshot)
    If Not rs.EOF Then
        lngID = rs!ThreadID
        rs.Close: Set rs = Nothing
    ...
    Set rs = db.OpenRecordset(TBL_THREADS, dbOpenDynaset)
    With rs
        .AddNew
...
Private Sub AktualisiereThread(db As DAO.Database, lngThreadID As Long, dtDatum As Date)
    ...
    Set rs = db.OpenRecordset( _
        "SELECT * FROM " & TBL_THREADS & " WHERE ThreadID=" & lngThreadID, dbOpenDynaset)
    If Not rs.EOF Then
        rs.Edit
...
Public Sub SpeichereEmailContent(ByVal lngEmailID As Long, _
    ...
    Set db = CurrentDb
    Set rs = db.OpenRecordset(TBL_CONTENT, dbOpenDynaset)
...
Public Sub SpeichereEmpfaenger(ByVal lngEmailID As Long, _
    ...
    Set db = CurrentDb
    Set rs = db.OpenRecordset(TBL_EMPFAENGER, dbOpenDynaset)
...
Public Function SpeichereAnhangMetadaten(ByVal lngEmailID As Long, _
    ...
    Set db = CurrentDb
    Set rs = db.OpenRecordset(TBL_ANHAENGE, dbOpenDynaset)
...
Public Sub AktualisiereAnhangPfad(ByVal lngAnhangID As Long, ByVal strDateiPfad As String)
    On Error GoTo ErrHandler
    CurrentDb.Execute "UPDATE " & TBL_ANHAENGE & " SET " & _
